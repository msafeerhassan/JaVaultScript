...
function initializePeerConnection() {
    ...
    peerConnection.onconnectionstatechange = () => {
        console.log(`WebRTC Connection Status Update: ${peerConnection.connectionState}`);
    ...
    peerConnection.oniceconnectionstatechange = () => {
        console.log(`WebRTC ICE Connection Update: ${peerConnection.iceConnectionState}`);
    };
...
async function acceptRemotePeerConnection() {
    ...
    try {
        ...
        await peerConnection.setRemoteDescription(new RTCSessionDescription(remoteDescriptionObject));
        console.log(`Remote Description Applied Successfuly! Type: ${remoteDescriptionObject.type}`);
...
async function executeAutomatedKeyExchange() {
    try {
        ...
        const rawSpkiBuffer = await exportPublicKeySpki(localEphemeralKeyPair.publicKey);
...
async function handleIncomingMsg(rawWireDate) {
    try {
        ...
        if(parsedFrame && parsedFrame.type === "KEY_EXCHANGE_PUBLIC_KEY"){
            ...
            const ifOfferer = peerConnection && peerConnection.localDescription && peerConnection.localDescription.type === "offer";
            ...
            if(ifOfferer){
                ...
                const wrappedKeyBuffer = await wrapSymmetricKey(freshSessionKey, remotePeerPublicKey);
        ...
        if(parsedFrame && parsedFrame.type === "KEY_EXCHANGE_SESSION_KEY"){
            ...
            const unwrappedSessionKey = await unwrapSymetricKey(wrappedBuffer, localEphemeralKeyPair.privateKey);
        ...
        if(parsedFrame && parsedFrame.type === "TEXT_MESSAGE"){
            if(!chatSession.key){
                renderMessage("Other", "Error: Cannot Decrypt Message. Local Session Locked");
                return;
            }

            const decryptedText = await decryptText(parsedFrame.ciphertext, parsedFrame.iv, chatSession.key);
            renderMessage(`Other: ${decryptedText}`);
        }
    } catch (e) {
